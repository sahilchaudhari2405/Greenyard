    ...
    <script>
        $(document).ready(function() {
            $('#email').on('change', function() {
                ...
                if (getEmail.length == 0) $('#email').css('border', 'red').after("<br><span>Email can't be empty!</span>");
                else {
                    $.ajax({
                        url: 'process.php',
                        type: 'post',
                        ...
                        success: function(response) {
                            ...
                            if (response == 'Reset authorised') {
                                alert(response);
                                window.location = 'setPassword.php';
                            } else $('#email').css('border', 'red').after("<br><span>Email doesn't match any record!</span>");
                        }
                    });
                }
            });
